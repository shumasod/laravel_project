...
@section('content')
<h2 style="margin-bottom: 2rem;">部屋編集</h2>
...
<div class="card">
    <form action="{{ route('rooms.update', $room) }}" method="POST">
        ...
        <div class="form-group">
            ...
            <select id="accommodation_id" name="accommodation_id" required>
                ...
                @foreach($accommodations as $accommodation)
                    <option value="{{ $accommodation->id }}" {{ old('accommodation_id', $room->accommodation_id) == $accommodation->id ? 'selected' : '' }}>
                        {{ $accommodation->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="form-group">
            ...
            <input type="text" id="room_type" name="room_type" value="{{ old('room_type', $room->room_type) }}" required>
            @error('room_type')
                <span style="color: red; font-size: 0.875rem;">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div class="form-group">
            ...
            <input type="number" id="price_per_night" name="price_per_night" value="{{ old('price_per_night', $room->price_per_night) }}" min="0" step="0.01" required>
